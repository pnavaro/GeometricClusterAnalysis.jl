# Algorithm DTM-Filtration from paper 'DTM-based Filtrations' Hirokazu Anai,
# Frederic Chazal, Marc Glisse, Yuichi Ike, Hiroya Inakoshi,Raphael Tinarrage,
# Yuhei Umeda

# and

# Algorithm based on the sublevel sets of the power function of the paper
...
distance_matrix_Power_function_Buchet <- function(birth, points) {
    ...
    for (i in 1:c) {
        for (j in 1:i) {
            distance_matrix[i, j] = hauteur(points[i, ], points[j, ], birth[i]^2,
                birth[j]^2)
        }
    }
    return(distance_matrix)
