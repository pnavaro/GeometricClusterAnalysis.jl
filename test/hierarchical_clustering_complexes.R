...
hierarchical_clustering_lem <- function(distance_matrix, infinity = Inf, threshold = Inf,
    ...
    # distance_matrix : (r_{i,j})_{i,j} r_{i,j} : time r when components i and
    # j merge r_{i,i} : birth time of component i.  c : number of components
    # infinity : components whose lifetime is larger than infinity never die
    # threshold : centers born after threshold are removed It is possible to
    # select infinity and threshold after running the algorithm with infinity =
    # Inf and threshold = Inf For this, we look at the persistence diagram of
    # the components : (x-axis Birth ; y-axis Death) store_colors = TRUE : in
    # the list Couleurs, we store all configurations of colors, for every step.
    # Thresholding :

    # Matrice_hauteur is modified such that diagonal elements are
    # non-decreasing
    ...
    for (i in 2:c) {
        for (j in 1:(i - 1)) {
            matrice_dist[i, j] = min(distance_matrix[startup_indices[i], startup_indices[j]],
    ...
    # ihi >= ihj since the matrix is triangular inferior with infinity value
    # above the diagonal

    while (continu) {
        if (timestep == matrice_dist[ihi, ihi]) {
            # Component ihi birth
            couleurs[ihi] = ihi
            matrice_dist[ihi, ihi] = Inf  # No need to get born any more
            indice = indice + 1
        } else {
            # Components of the same color as ihi and of the same color as ihj
            # merge
            coli0 = couleurs[ihi]
            colj0 = couleurs[ihj]
            coli = max(coli0, colj0)
            colj = min(coli0, colj0)
            if (timestep - birth[coli] <= infinity) {
                # coli and colj merge NB ihi<=indice, so couleurs[ihi] =
                # couleurs[ihj]
                for (i in 1:min(indice, c)) {
                  if (couleurs[i] == coli) {
                    couleurs[i] = colj
                ...
                # Component coli dont die, since lives longer than infinity.
                # NB ihi<=indice, so couleurs[ihi] = couleurs[ihj]
                for (i in 1:min(indice, c)) {
                  if (couleurs[i] == coli) {
                    for (j in 1:min(indice, c)) {
                      if (couleurs[j] == colj) {
    ...
    return(list(color = couleurs, Couleurs = Couleurs, Temps_step = Temps_step, birth = birth,
        death = death, startup_indices = startup_indices))
}

build_distance_matrix <- function(means, weights, cov_matrices, indexed_by_r2 = TRUE) {
    # means: matrix of size cxd weights: vector of size c cov_matrices: list of
    # c symmetric matrices of size dxd indexed_by_r2 = TRUE always work ;
    # indexed_by_r2 = FALSE requires elements of weigts to be non-negative.
    # indexed_by_r2 = FALSE for the sub-level set of the square-root of
    # non-negative power functions : the k-PDTM or the k-PLM (when determinant
    # of matrices are forced to be 1)
    c = nrow(means)
    if (c != length(weights)) {
        return("The number of rows of means should be equal to the length of weights")
    }
    ...
    if (c == 1) {
        if (indexed_by_r2 == TRUE) {
            ...
            # Indexed by r -- only for non-negative functions (k-PDTM and k-PLM
            # with det = 1)
    ...
    for (i in 2:c) {
        for (j in 1:(i - 1)) {
            distance_matrix[i, j] = intersection_radius(cov_matrices[[i]], cov_matrices[[j]],
                means[i, ], means[j, ], weights[i], weights[j])
        }
    }
    if (indexed_by_r2 == TRUE) {
...
return_color <- function(centre, couleurs, startup_indices) {
    # centre : vector of integers such that centre[i] is the label of the
    # center associated to the i-th point couleurs[1] : label of the center
    # that is born first, i.e. for the Indice_depart[1]-th center
...
second_passage_hc <- function(dist_func, distance_matrix, infinity = Inf, threshold = Inf,
    indexed_by_r2 = TRUE, store_colors = FALSE, store_timesteps = FALSE) {
    ...
    hc = hierarchical_clustering_lem(distance_matrix, infinity = infinity, threshold = threshold,
        store_colors, store_timesteps)
    # Transforming colors # Problem : color contains less than sig signal
    # points...
...
color_points_from_centers <- function(P, k, sig, dist_func, hc, plot = FALSE) {
    ...
    if (plot) {
        plot_pointset_centers_ellipsoids_dim2(P, color_final, dist_func$means[remain_indices,
            ], dist_func$weights[remain_indices], matrices, 0, color_is_numeric = FALSE,
            fill = FALSE)
    }
    return(color_final)
}

color_points_from_centers_2 <- function(P, k, sig, centers, Sigma, means, weights,
    hc, plot = FALSE) {
    Col = hc$color
    remain_indices = hc$startup_indices
    matrices = list()
...
plot_birth_death <- function(hierarchical_clustering, lim_min = 0, lim_max = 1, filename = "persistence_diagram.pdf",
    path = "results/", plot = TRUE) {
    ...
    Death = hcdeath
    if (plot) {
        gp = ggplot() + geom_point(aes(x = Birth, y = Death), col = "black") + geom_line(aes(grid,
            grid))
        print(gp)
