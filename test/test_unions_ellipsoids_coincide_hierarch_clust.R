# For the different functions (k-PLM, k-PDTM), test the second_passage_hc
# algorithm.

# Thanks to the function plot_all_steps : We observe the evolution of the
# colors of the points in the different steps of the algorithm.  In particular,
# points that are outside the union of ellipsoids are colored in pink.  And
...
plot_all_steps <- function(method, P, k, c, sig, iter_max, nstart, infinity = Inf,
    threshold = Inf) {
    ...
    distance_matrix = build_distance_matrix(dist_func$means, dist_func$weights, dist_func$Sigma,
        indexed_by_r2 = TRUE)
    fp_hc = second_passage_hc(dist_func, distance_matrix, infinity = infinity, threshold = threshold,
        indexed_by_r2 = TRUE, store_colors = TRUE, store_timesteps = TRUE)
    ...
    compute_color_value = colorize(P, k, sig, dist_func$centers[remain_indices, ],
        matrices)
    ...
    for (i in 1:length(Colors)) {
        plot_pointset_centers_ellipsoids_dim2(P, Colors[[i]], res$means[remain_indices,
            ], res$weights[remain_indices], matrices, Temps[[i]], color_is_numeric = FALSE,
            fill = FALSE)
        # plot_pointset_centers_ellipsoids_dim2(P,Colors[[i]],res$means,res$weights,res$Sigma,Temps[[i]],color_is_numeric
        # = FALSE,fill = FALSE) # To plot every ellipsoids
    }
}
...
## With the k-PDTM method = function(P,k,c,sig,iter_max,nstart){
## return(kpdtm(P,k,c,sig,iter_max,nstart)) }
## plot_all_steps(method,P,k,c,sig,iter_max,nstart,infinity = Inf, threshold =
## 5)
