...
colorize <- function(P,k,sig,centers,Sigma){
  ...
  for(i in 1:c){
    ...
    weights[i] = mean(mahalanobis(P[nn$ix,],means[i,],Sigma[[i]],inverted = FALSE)) + log(det(Sigma[[i]]))
  }
  # Step 2 : Update color
  ...
  for(j in 1:N){
    ...
    for(i in 1:nrow(centers)){
      newcost = mahalanobis(P[j,],means[i,],Sigma[[i]],inverted = FALSE)+weights[i]
      if(newcost<=cost){
        cost = newcost
        best_ind = i
      }
...
LL_minimizer_multidim_trimmed_lem <- function(P,k,c,sig,iter_max = 10,nstart = 1,f_Sigma = function(Sigma){return(Sigma)}){
# P : an Nxd matrix
# k : number of neigbours to consider, in 2..N
# c : number of ellipsoids, in 1..N
# sig : number of points in P considered as signal points by the algorithm, in 0..N
...
# f_sigma : a function to transform the matrix Sigma (for instance, to force it to have a determinant = 1, or equal eigenvalues...)
  ...
  opt = list(    cost = Inf,
                 ...
                 Sigma_inverse = rep(list(diag(1,d)),c), # Contains the inverses of the matrices
                 color = rep(0,N),
  ...
  for(n_times in 1:nstart){
    ...
    new = list(  cost = Inf,
                 ...
                 color = rep(0,N),
                 kept_centers = rep(TRUE,c),
                 means = matrix(data=0,nrow=c,ncol=d), # means of \tilde P_{\theta_i,h}
                 weights = rep(0,c)
    )
    Nstep = 0
    continu_Sigma = TRUE
    ...
    while((continu_Sigma||(!(all.equal(old$centers,new$centers)==TRUE)))&&(Nstep<=iter_max)){
      ...
      for(i in 1:c){
        nn = sort(mahalanobis(P,old$centers[i,],old$Sigma_inverse[[i]],inverted = TRUE),index.return=TRUE)
        nn$x = nn$x[1:k]
        nn$ix = nn$ix[1:k]
        new$means[i,] = colMeans(matrix(P[nn$ix,],k,d))
        new$weights[i] = mean(mahalanobis(P[nn$ix,],new$means[i,],old$Sigma_inverse[[i]],inverted = TRUE)) - log(det(old$Sigma_inverse[[i]]))
      }
      ...
      for(j in 1:N){
        ...
        for(i in 1:c){
          if(new$kept_centers[i]){
            newcost = mahalanobis(P[j,],new$means[i,],old$Sigma_inverse[[i]],inverted = TRUE)+new$weights[i]
            if(newcost<=cost){
              cost = newcost
              best_ind = i
            }
      ...
      for(i in 1:c){
        ...
        if(nb_points_cloud>1){
          ...
          nn = sort(mahalanobis(P,new$centers[i,],old$Sigma_inverse[[i]],inverted = TRUE),index.return=TRUE)
          nn$x = nn$x[1:k]
          nn$ix = nn$ix[1:k]
          new$means[i,] = colMeans(matrix(P[nn$ix,],k,d))
          aux = ((new$means[i,]-new$centers[i,]) %*% t(new$means[i,]-new$centers[i,])) + ((k-1)/k)*cov(P[nn$ix,]) + ((nb_points_cloud-1)/nb_points_cloud)*cov(P[new$color==i,])
          ...
          if(nb_points_cloud==1){
            ...
            nn$ix = nn$ix[1:k]
            new$means[i,] = colMeans(matrix(P[nn$ix,],k,d))
            aux = ((new$means[i,]-new$centers[i,]) %*% t(new$means[i,]-new$centers[i,])) + ((k-1)/k)*cov(P[nn$ix,]) #+0 (car un seul element dans C)
      ...
      for (i in 1:c){
        if(new$kept_centers[i]){
          stop_Sigma = stop_Sigma*(all.equal(new$Sigma_inverse[[i]],old$Sigma_inverse[[i]])==TRUE)
        }
      }
