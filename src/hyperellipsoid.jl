...
struct HyperEllipsoid
    ...
    axis :: Vector{Vector{Bool}}
    extent :: Vector{Float64}

    function HyperEllipsoid(dim)
        center = zeros(dim)
        axis = [1:dim .== i for i in 1:dim]
        extent = ones(dim)
        new(dim, center, axis, extent)
    end

end

function Base.print(io::IO, he::HyperEllipsoid)
    print( IOContext(io, :limit => true),
        "HyperEllipsoid:
center = $(he.center)
axis = [\n",
        he.axis...,
...
Base.show(io::IO, model::HyperEllipsoid) = print(io, model)
