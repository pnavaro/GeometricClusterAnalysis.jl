                    ...
                    if (U <= threshold[5]) {
                      ...
                      if (U <= threshold[6]) {
                        ...
                        if (U <= threshold[7]) {
                          ...
                          if (U <= threshold[8]) {
                            col[i] = 8
                            P[i, 1] = P[i, 1] + V * cos(2 * pi/7) + (1 - V) * cos(4 *
                              pi/7)
                            P[i, 2] = P[i, 2] + V * sin(2 * pi/7) + (1 - V) * sin(4 *
                              pi/7)
                          } else {
                            if (U <= threshold[9]) {
                              col[i] = 9
                              P[i, 1] = P[i, 1] + V * cos(4 * pi/7) + (1 - V) * cos(6 *
                                pi/7)
                              P[i, 2] = P[i, 2] + V * sin(4 * pi/7) + (1 - V) * sin(6 *
                                pi/7)
                            } else {
                              if (U <= threshold[10]) {
                                col[i] = 10
                                P[i, 1] = P[i, 1] + V * cos(6 * pi/7) + (1 - V) *
                                  cos(8 * pi/7)
                                P[i, 2] = P[i, 2] + V * sin(6 * pi/7) + (1 - V) *
                                  sin(8 * pi/7)
                              } else {
                                if (U <= threshold[11]) {
                                  col[i] = 11
                                  P[i, 1] = P[i, 1] + V * cos(8 * pi/7) + (1 - V) *
                                    cos(10 * pi/7)
                                  P[i, 2] = P[i, 2] + V * sin(8 * pi/7) + (1 - V) *
                                    sin(10 * pi/7)
                                } else {
                                  if (U <= threshold[12]) {
                                    col[i] = 12
                                    P[i, 1] = P[i, 1] + V * cos(10 * pi/7) + (1 -
                                      V) * cos(12 * pi/7)
                                    P[i, 2] = P[i, 2] + V * sin(10 * pi/7) + (1 -
                                      V) * sin(12 * pi/7)
                                  } else {
                                    if (U <= threshold[13]) {
                                      col[i] = 13
                                      P[i, 1] = P[i, 1] + V * cos(12 * pi/7) + (1 -
                                        V) * cos(14 * pi/7)
                                      P[i, 2] = P[i, 2] + V * sin(12 * pi/7) + (1 -
                                        V) * sin(14 * pi/7)
                                    } else {
                                      col[i] = 14
                                      P[i, 1] = P[i, 1] + V * cos(14 * pi/7) + (1 -
                                        V) * cos(2 * pi/7)
                                      P[i, 2] = P[i, 2] + V * sin(14 * pi/7) + (1 -
                                        V) * sin(2 * pi/7)
                                    }
                                  }
                                }
...
generate_14segments_noise <- function(N, Nnoise, sigma, dim) {
    # N : number of signal points Nnoise : number of additionnal outliers --
    # sampled accordingly to generate_noise Signal points are X = Y+Z with Y
    # uniform on the 14 segments Z normal with mean 0 and covariance matrix
    # sigma*I_dim (with I_dim the identity matrix of R^dim) So, dim is the
    # dimension of the data and sigma, the standard deviation of the additive
    # Gaussian noise.  When dim>2, Y_i = 0 for i>=2 ; with the notation
    # Y=(Y_i)_{i=1..dim}
